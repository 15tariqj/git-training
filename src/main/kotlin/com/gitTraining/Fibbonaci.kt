...
fun computeFibbonaciNumber(position: Int?, recursion: Boolean = false): Int {
    ...
    if (notNullPosition < 0) {
        val notNullPositionIsOdd = notNullPosition % 2 == -1
        return if (notNullPositionIsOdd) computeFibbonaciNumber(-notNullPosition) else (computeFibbonaciNumber(-notNullPosition) * -1)}
